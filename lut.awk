BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["LUT_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["LUT_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["LUT_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["LUT_NOLINEWRAP"] != "" && ENVIRON["LUT_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
